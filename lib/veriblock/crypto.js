...
exports.SIGNATURE_ASN1_PREFIX = Buffer.from('304502204348CE', 'hex');
class PublicKey {
    constructor(buffer) {
        if (buffer.length === 88 &&
            buffer[23] === 0x04 &&
    ...
    get address() {
        return address.addressFromPublicKey(this);
    }
}
...
class Signature {
    constructor(buffer) {
        if (buffer.length === 64) {
            ...
            this._full = Buffer.concat([exports.SIGNATURE_ASN1_PREFIX, buffer]);
        }
        else if (buffer.length === 71 &&
            buffer
                .slice(0, exports.SIGNATURE_ASN1_PREFIX.length)
                .compare(exports.SIGNATURE_ASN1_PREFIX) === 0) {
...
class KeyPair {
    ...
    static generate(entropy) {
        ...
        const priv = new PrivateKey(Buffer.from(zeltrezcrypto.sha256(entropy), "hex"));
...
class SHA256withECDSA {
    static sign(msg, privateKey) {
        ...
        const m = Buffer.from(zeltrezcrypto.sha256(msg), "hex");
