...
function omniToScript(omniHex) {
  // is already parsed as hex
  return zopcodes.OP_RETURN + zbufferutils.getPushDataLength(omniHex) + omniHex;
}
 ...
 */
function createRawTx(history, recipients, omni) {
  ...
  txObj.outs.push({
    script: omniToScript(omni.omniHex)
  });
