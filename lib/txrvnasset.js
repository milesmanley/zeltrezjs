 ...
 */
function mkPubkeyHashReplayScript(address, pubKeyHash, assetSatoshi, assetName) {
  ...
  if (assetSatoshi > 0) {
    ...
    var arr = [];
    for (let i = 0, l = assetName.length; i < l; i++) {
      var hex = Number(assetName.charCodeAt(i)).toString(16);
      arr.push(hex);
    }
    var assetNameHex = arr.join('');

    var assetSatoshiHex = assetSatoshi.toString(16);
    if (assetSatoshiHex.length % 2 != 0) {
      assetSatoshiHex = '0' + assetSatoshiHex;
    }
    var arrayAssetSatoshi = [];
    for (let i = 0; i < assetSatoshiHex.length; i++) {
      const ash = assetSatoshiHex.substring(i, i + 2);
      arrayAssetSatoshi.unshift(ash);
      i++;
    }
    var assetSatoshiHexReversed = arrayAssetSatoshi.join('');

    const k = (16 - assetSatoshiHexReversed.length) / 2;
    var nullString = '';
    for (let j = 0; j < k; j++) {
      nullString += zopcodes.OP_0;
    }

    return zopcodes.OP_DUP + zopcodes.OP_HASH160 + zbufferutils.getPushDataLength(subAddrHex) + subAddrHex + zopcodes.OP_EQUALVERIFY + zopcodes.OP_CHECKSIG + zopcodes.OP_RVN_ASSET + zbufferutils.getPushDataLength(zopcodes.RVN_R + zopcodes.RVN_V + zopcodes.RVN_N + zopcodes.RVN_T + zbufferutils.getPushDataLength(assetNameHex) + assetNameHex + assetSatoshiHexReversed + nullString) + zopcodes.RVN_R + zopcodes.RVN_V + zopcodes.RVN_N + zopcodes.RVN_T + zbufferutils.getPushDataLength(assetNameHex) + assetNameHex + assetSatoshiHexReversed + nullString + zopcodes.OP_DROP;
  }
  // Minimal encoding
 ...
 */
function mkScriptHashReplayScript(address, pubKeyHash, assetSatoshi, assetName) {
  ...
  if (assetSatoshi > 0) {
    ...
    return zopcodes.OP_HASH160 + zbufferutils.getPushDataLength(subAddrHex) + subAddrHex + zopcodes.OP_EQUAL + zopcodes.OP_RVN_ASSET + zbufferutils.getPushDataLength(zopcodes.RVN_R + zopcodes.RVN_V + zopcodes.RVN_N + zopcodes.RVN_T + zbufferutils.getPushDataLength(assetNameHex) + assetNameHex + assetSatoshiHexReversed + nullString) + zopcodes.RVN_R + zopcodes.RVN_V + zopcodes.RVN_N + zopcodes.RVN_T + zbufferutils.getPushDataLength(assetNameHex) + assetNameHex + assetSatoshiHexReversed + nullString + zopcodes.OP_DROP;
  }
 ...
 */
function addressToScript(address, pubKeyHash, isP2SH, assetSatoshi, assetName) {
  ...
  if (isP2SH) {
    return mkScriptHashReplayScript(address, pubKeyHash, assetSatoshi, assetName);
  } else {
    // P2PKH-replay is a replacement for P2PKH
    // P2PKH-replay starts with a 0
    return mkPubkeyHashReplayScript(address, pubKeyHash, assetSatoshi, assetName);
  }
}
...
function createRawTx(history, recipients) {
  ...
  txObj.outs = recipients.map(function (o) {
    this.pubKeyHash;
    return {
      script: addressToScript(o.address, o.pubKeyHash, o.isP2SH, o.assetSatoshi, o.assetName),
      satoshis: o.satoshis
    };
  });
