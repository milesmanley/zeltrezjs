...
function addressToScript(address, pubKeyHash, isP2SH) {
  ...
  if (pubKeyHash === undefined)
    pubKeyHash = zconfig.mainnet.scriptPubKey
...
function deserializeTx(hexStr) {
  ...
  // Version
  txObj.version = buf.readInt8(offset);

  //skip past version 1 byte, txtype 1 byte, iswitness 1 byte
  offset += 3;
  ...
  for (var i = 0; i < voutLen; i++) {
    // skip forward to bypass the txout type
    offset += 1;
...
function serializeTx(txObj) {
  ...
  // Build the first 6 bytes of the Tx
  // Veil - Add the Tx version currently - 02
  serializedTx += '02'
  // Veil - Followed by the transaction type 00 - standard
  serializedTx += '00' 
  // Veil - Followed by if this tx has any witnesses - 00 for no
  serializedTx += '00' 

  // Veil - Add the LockTime
  ...
  txObj.outs.map(function (o) {
    ...
    // Veil - Add the TxOut version 01 - basecoin
    serializedTx += '01';
...
/*
 * Serializes a TXOBJ into hex string used for signing transactions for signatures
 * @param {Object} txObj
 * return {String} hex string of txObj
 */
function serializeTxForSig(txObj) {
  var serializedTx = '';
  var _buf16 = Buffer.alloc(4);
  var _buf8 = Buffer.alloc(2);
...
function createRawTx(history, recipients) {
  var txObj = { locktime: 0, version: 2, ins: [], outs: [] };
...
function signTx(_txObj, i, privKey, compressPubKey, hashcode) {
  ...
  const signingTxHex = serializeTxForSig(signingTx);
...
module.exports = {
  ...
  serializeTxForSig: serializeTxForSig,
  signTx: signTx
};